    ...
    function walk(currentDir) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                if (["node_modules", ".git", "build", "target", ".gradle", ".idea", ".cxx", "dist"].includes(entry.name)) {
                    continue;
                }
                walk(fullPath);
            }
...
function scanFile(filePath, content, rules, projectDir) {
    const findings = [];
                ...
                if (source.includes("[\\s\\S]") || source.includes("[\\S\\s]")) {
                    ...
                    for (let offset = 0; offset < content.length; offset += chunkSize - 500) {
                        ...
                        if (bp.pattern.test(chunk)) {
                            ...
                            const isDuplicate = findings.some((f) => f.ruleId === rule.id && f.file === relPath && Math.abs(f.line - approxLine) < 10);
                            if (!isDuplicate) {
                                findings.push({
                                    ruleId: rule.id,
                                    ruleName: rule.name,
...
export function audit(projectDir, options) {
    const rules = allRules.filter((r) => {
        if (options?.categories && !options.categories.includes(r.category))
            return false;
        if (options?.severity && !options.severity.includes(r.severity))
            return false;
        return true;
    });
    ...
    for (const file of files) {
        ...
        const applicableRules = rules.filter((r) => r.filePatterns.some((p) => {
