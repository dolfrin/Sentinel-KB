...
// ─── Tool: Full audit ───────────────────────────────────────────
server.tool("audit", "Run a full messenger security audit on a project directory. Scans for E2E encryption issues, WebRTC/P2P vulnerabilities, messenger-specific flaws, Android security, and Rust backend problems.", {
...
// ─── Tool: List rules ───────────────────────────────────────────
server.tool("list-rules", "List all available security audit rules with their IDs, categories, and severity levels.", {
...
server.tool("scan-file", "Scan a single file against all messenger security rules.", {
    ...
    try {
        ...
        const fileFindings = report.findings.filter((f) => f.file === path.basename(filePath) || filePath.endsWith(f.file));
        if (fileFindings.length === 0) {
            return { content: [{ type: "text", text: `✅ No findings in ${filePath}` }] };
        }
        const lines = [`Findings in ${filePath}:\n`];
